...
export default function UnderTab() {
  return (
    <div className='bg-white bg-opacity-95 fixed bottom-0 h-[40vh] w-full rounded-t-2xl text-zinc-800 left-0'>
      ...
      <div className='pt-14 flex flex-col gap-4'>
        ...
        <div>
          <p className='font-bold underline text-center' onClick={() => {
            window.location.href = '/auth';
          }}>メールアドレスで登録</p>
        </div>
        <div>
          <p className='text-center text-sm text-zinc-400'>アカウントをお持ちでない場合は、新規登録してください。</p>
        </div>
      </div>
    </div>
  );
